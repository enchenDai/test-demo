    ...
    templateUrl: null
        ...
        return {
            ...
            controller: ['$scope', '$templateCache', '$interpolate', 'treeConfig', function ($scope, $templateCache, $interpolate, treeConfig) {
                    ...
                    ensureDefault($scope.options, "isSelectedProperty", "isSelected");
                    ...
                    $scope.indentStyle = function (node) {
                        return { 'margin-left': (node.indentLevel || 0) * 32 + 'px' };
                    };
                    ...
                    $scope.isSelectedNode = function() {
                        return isSelectedNode(this.node);
                    }
                        ...
                        if (!$scope.options.isLeaf(selectedNode) && (!$scope.options.dirSelectable || !$scope.options.isSelectable(selectedNode))) {
                            ...
                            if ($scope.options.multiSelection) {
                                ...
                                if (pos === -1) {
                                    ...
                                    selectedNode[$scope.options.isSelectedProperty] = true;
                                    selected = true;
                                } else {
                                    $scope.selectedNodes.splice(pos, 1)[0][$scope.options.isSelectedProperty] = false;
                                }
                            } else {

                                if ($scope.selectedNode)
                                    $scope.selectedNode[$scope.options.isSelectedProperty] = false;
                                ...
                                if (selected)
                                    $scope.selectedNode[$scope.options.isSelectedProperty] = true;
                            }
                    ...
                    if (!template) {
                        template =
                            ...
                            'set-node-to-data> <span ng-style="indentStyle(node)"></span>' +
            ...
            compile: function (element, attrs, childTranscludeFn) {
                return function (scope, element, attrs, treemodelCntr) {
                    ...
                    function setIndentLevel(node, level)
                    {
                        ...
                        angular.forEach(children, function (childNode) { setIndentLevel(childNode, level + 1); });
                    }

                    scope.$watch("treeModel", function updateNodeOnRootScope(newValue) {
                        if (angular.isArray(newValue)) {
                            if (scope.node && angular.isDefined(scope.node) && angular.equals(scope.node[scope.options.nodeChildren], newValue))
                                return;
                            scope.node = {};
